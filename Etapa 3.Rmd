...
p <- datos$p26
p <- suppressWarnings(as.numeric(trimws(if (is.factor(p)) as.character(p) else p)))
g <- datos$sexo
g <- if (is.factor(g)) as.character(g) else g
g <- ifelse(g=="1","Hombre", ifelse(g=="2","Mujer", NA))

ok <- !is.na(p) & !is.na(g); x <- p[ok]; g <- g[ok]

ord <- c("Hombre","Mujer"); sp <- split(x, g)
n <- sapply(ord, function(k) length(sp[[k]]))
m <- sapply(ord, function(k) mean(sp[[k]]))
s <- sapply(ord, function(k) sd(sp[[k]]))
tcrit <- qt(1 - alpha/2, n - 1)
e <- tcrit * s / sqrt(n); LI <- m - e; LS <- m + e
mx <- mean(x)

xr <- range(c(LI, LS), na.rm=TRUE); mg <- 0.05*diff(xr); xr <- xr + c(-mg, mg)
plot(0, ylim=c(0,3), xlim=xr, yaxt="n", ylab="", xlab="Minutos de lectura (p26)",
main="Minutos por sexo (IC 96%)")
axis(2, at=1:2, labels=ord)
arrows(LI[1],1,LS[1],1, angle=90, code=3, length=0.1, lwd=2, col="orange")
arrows(LI[2],2,LS[2],2, angle=90, code=3, length=0.1, lwd=2, col="blue")
points(m[1],1,pch=19,cex=1.1,col="orange"); points(m[2],2,pch=19,cex=1.1,col="blue")
abline(v=mx, col="red")
